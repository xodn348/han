...
use super::value::{RuntimeError, Signal, Value};
use crate::ast::{BinaryOpKind, Expr, Pattern, Stmt, StmtKind, UnaryOpKind};
        ...
        Expr::Call { name, args } => {
            ...
            match func_val {
                Value::Function { params, body } => {
                    ...
                    // Preserve original copy-on-call semantics: snapshot caller bindings
                    // into a fresh isolated frame so mutations inside the function
                    // don't leak back to caller scope.
                    let func_env = Environment::new_ref();
                    let snapshot = env.borrow().snapshot();
                    {
                        let mut fe = func_env.borrow_mut();
                        for (k, v) in snapshot {
                            fe.set(k, v);
                        }
                        for ((param_name, _ty), val) in params.iter().zip(arg_vals) {
                            fe.set(param_name.clone(), val);
                        }
                    }
                ...
                Value::Closure {
                    ...
                    let closure_env = Environment::new_enclosed(captured);
                    {
                        let mut ce = closure_env.borrow_mut();
                        for ((param_name, _), val) in params.iter().zip(arg_vals) {
                            ce.set(param_name.clone(), val);
                        }
                    }
    ...
    match obj {
        ...
        Value::Struct {
            ...
            if let Some(func_val) = func_val {
                match func_val {
                    Value::Function { params, body } => {
                        let method_env = Environment::new_ref();
                        let funcs = env.borrow().collect_functions();
                        {
                            let mut me = method_env.borrow_mut();
